...
export default function CMSLogin() {
  const { loginAdmin, companyInfo, supabaseConnected } = useCMS();
  const navigate = useNavigate();

  const [email, setEmail] = useState('admin@intaichinh.vn');
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.success) {
        setErrorMsg(res.error || 'Đăng nhập thất bại. Vui lòng kiểm tra lại tài khoản và mật khẩu.');
        setLoading(false);
        return;
      }
  ...
  const handleQuickPreset = (presetEmail: string, presetPass: string = '123456') => {
    ...
    <div className="min-h-screen bg-[#F7FAFF] flex flex-col justify-between relative overflow-hidden font-sans text-slate-800 selection:bg-[#174A87] selection:text-white">
      ...
      <header className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex justify-between items-center relative z-10">
        ...
        <div className="flex items-center gap-3">
          <div className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-bold bg-white border border-[#DCE7F2] text-slate-600 shadow-sm">
            <Database className={`w-3.5 h-3.5 ${supabaseConnected ? 'text-emerald-600' : 'text-amber-500'}`} />
            <span>{supabaseConnected ? 'Supabase Database: Đã kết nối' : 'Chế độ Trực quan / Demo'}</span>
          </div>
        ...
        >
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-xs text-slate-500 mt-2 font-normal leading-relaxed">
              Hệ thống xác thực Supabase & phân quyền bảo mật dành cho Quản trị viên, Người duyệt bài và Biên tập viên.
            </p>
          </div>
          ...
          <form onSubmit={handleLoginSubmit} className="space-y-5">
            ...
            <div className="flex items-center justify-between pt-1">
              <label className="flex items-center gap-2 cursor-pointer select-none">
                ...
                />
                <span className="text-xs text-slate-600 font-medium">Duy trì phiên đăng nhập</span>
              </label>
            </div>
          ...
          {/* Quick presets for the 3 requested roles */}
          <div className="mt-8 pt-6 border-t border-[#DCE7F2]">
            <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider font-display mb-3 flex items-center justify-center gap-1.5">
              ...
              Chọn Nhanh Tài Khoản Thử Nghiệm Phân Quyền
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-[11px]">
              <button
                type="button"
                onClick={() => handleQuickPreset('admin@intaichinh.vn')}
                className="p-2 bg-[#174A87]/10 hover:bg-[#174A87]/20 text-[#174A87] rounded-xl border border-[#174A87]/20 font-bold transition-all text-center flex flex-col items-center gap-1 cursor-pointer"
              >
                <ShieldAlert className="w-4 h-4" />
                <span>Quản Trị Viên</span>
                <span className="text-[9px] font-normal text-slate-500">Toàn quyền</span>
              </button>
              <button
                type="button"
                onClick={() => handleQuickPreset('duyetbai@intaichinh.vn')}
                className="p-2 bg-amber-50 hover:bg-amber-100 text-amber-800 rounded-xl border border-amber-200 font-bold transition-all text-center flex flex-col items-center gap-1 cursor-pointer"
              >
                <UserCheck className="w-4 h-4" />
                <span>Người Duyệt Bài</span>
                <span className="text-[9px] font-normal text-amber-700">Duyệt & Đăng</span>
              </button>
              <button
                type="button"
                onClick={() => handleQuickPreset('bientap@intaichinh.vn')}
                className="p-2 bg-emerald-50 hover:bg-emerald-100 text-emerald-800 rounded-xl border border-emerald-200 font-bold transition-all text-center flex flex-col items-center gap-1 cursor-pointer"
              >
                <FileEdit className="w-4 h-4" />
                <span>Biên Tập Viên</span>
                <span className="text-[9px] font-normal text-emerald-700">Tạo & Gửi duyệt</span>
              </button>
            </div>
            <p className="text-[10px] text-slate-400 mt-3 text-center">Mật khẩu mẫu: <code className="font-mono font-bold bg-slate-100 px-1.5 py-0.5 rounded text-slate-700">123456</code> (Hoặc sử dụng tài khoản tạo từ Supabase Auth)</p>
          </div>
        </motion.div>
      </main>
      ...
      <footer className="w-full max-w-7xl mx-auto px-4 py-6 text-center text-xs text-slate-500 relative z-10 border-t border-[#DCE7F2]/60">
        <p className="font-semibold text-slate-600">
          &copy; {new Date().getFullYear()} {companyInfo.name} — HỆ THỐNG CMS QUẢN TRỊ NỘI DUNG SUPABASE
        </p>
        <p className="text-[10px] text-slate-400 mt-1">
          Bảo mật đa tầng | Phân quyền RLS Row Level Security | Chuẩn ISO 9001:2015
        </p>
      </footer>
